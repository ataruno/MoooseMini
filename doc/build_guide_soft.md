...
- [キーマップ](#キーマップ)
  - [デフォルト設定や固定のもの](#デフォルト設定や固定のもの)
  - [(1) ZMK Studioを使用する更新方法](#1-zmk-studioを使用する更新方法)
  - [(2) ZMK Firmwareを使用する更新方法](#2-zmk-firmwareを使用する更新方法)
...
よっぽど大丈夫だと思いますが、もし、うまく書き込めていなさそうな場合はMoooseMiniのuf2を上記のソフト書き込み手順に従いもう一度書き込みをしてみてください。  
...
* レイヤー0...デフォルトのマウス操作
* レイヤー1...ブラウザタブの移動などの操作
* レイヤー2...矢印キー操作
* レイヤー3...GUI+矢印キー操作(=画面の位置操作)
* レイヤー4...Bluetooth設定
...
TACT...レイヤー4  
を意味します。  
LCはLeftCtrl,LSはLeftShift,LGはLeftGUI(windowsキー)の組み合わせです。  
例えばLC(S)の場合、Ctrl+Sで上書きショートカットのイメージです。  

#### 固定のもの(=ZMK studioでは変更不可のもの)
ロータリーエンコーダの回転  
* レイヤー0...スクロールダウン/スクロールアップ  
* レイヤー1...Ctrl+Shift+Tab/Ctrl+Tab (タブ移動)  
* レイヤー2...音量減/音量増  
* レイヤー3...スクロール左/スクロール右  
* レイヤー4...スクロールダウン/スクロールアップ  

トラックボール操作の挙動  
* レイヤー0...マウスカーソル操作  
* レイヤー1...低感度マウスカーソル操作(細かな操作用)  
* レイヤー2...スクロールダウン/スクロールアップ  
* レイヤー3...スクロールダウン/スクロールアップ  
* レイヤー4...低感度マウスカーソル操作(細かな操作用)  
